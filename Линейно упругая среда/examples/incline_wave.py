...
for i in range(Ny):
    ...
    for j in range(Nx):
        ...
        #n = (1, 3)
        #if( -n[0]/n[1] * j + (Nx*1.5/3) <= i <= - n[0]/n[1] * j + (Nx*2/3) ):
        #n = (1, 1)
        #if( -n[0]/n[1] * j + (Nx-3) <= i <= - n[0]/n[1] * j + (Nx+3) ):

            Vx0 = n[0]/sqrt(n[0]**2 + n[1]**2)
            Vy0 = n[1]/sqrt(n[0]**2 + n[1]**2)

            c3 = cp - 2.0 * cs * cs / cp
            Gxx0 = rho * c3
            Gyy0 = rho * c3

            Gxx0 += rho*(cp - c3) * Vx0**2
            Gxy0 += rho*(cp - c3) * Vx0*Vy0
            Gyy0 += rho*(cp - c3) * Vy0**2
